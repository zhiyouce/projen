...
import { DependabotOptions } from './dependabot';
...
import { PeerDependencyOptions } from '.';

export interface PythonProjectCommonOptions {
  ...
  /**
   * Python.js version to require via package.json `engines` (inclusive).
   * @default - no "engines" specified
   */
  readonly minPythonVersion?: string;

  /**
   * Minimum Python.js version to require via `engines` (inclusive).
   * @default - no max
   */
  readonly maxPythonVersion?: string;

  /**
   * The Python version to use in GitHub workflows.
   * @default - same as `minPythonVersion`
   */
...
export interface PythonProjectOptions extends PythonProjectCommonOptions {
  /**
   * This is the name of your package. It gets used in URLs, as an argument on the command line,
   * and as the directory name inside Python_modules.
...
//   private readonly _version: string;
   ...
   */
  protected readonly buildWorkflow?: PythonBuildWorkflow;

  /**
   * The release GitHub workflow. `undefined` if `releaseWorkflow` is disabled.
   */
  protected readonly releaseWorkflow?: PythonBuildWorkflow;

  public readonly minPythonVersion?: string;
  public readonly maxPythonVersion?: string;
...
//   public readonly start?: Start;
  ...
  constructor(options: PythonProjectOptions) {
    ...
    // this._version = '1.0';
    this.minPythonVersion = options.minPythonVersion;
    this.maxPythonVersion = options.maxPythonVersion;
    ...
    if (this.minPythonVersion) {
      PythonVersion += `>= ${this.minPythonVersion}`;
    }

    if (this.maxPythonVersion) {
      PythonVersion += ` <= ${this.maxPythonVersion}`;
    }
    ...
    this.manifest = {
      ...
      'engines': PythonVersion !== '' ? { Python: PythonVersion } : undefined,
    };
    ...
    // }
    this.addScript('projen', `Python ${PROJEN_RC}`);
    ...
    this.PythonVersion = options.workflowPythonVersion ?? this.minPythonVersion;

    if (options.buildWorkflow ?? true) {
      this.buildWorkflow = new PythonBuildWorkflow(this, 'Build', {
        trigger: {
          pull_request: { },
        },
    ...
    if (options.releaseWorkflow ?? true) {
      ...
      this.releaseWorkflow = new PythonBuildWorkflow(this, 'Release', {
        trigger,
        bump: true,
        uploadArtifact: true,
        image: options.workflowContainerImage,
  ...
  public get workflowBootstrapSteps() {
    const PythonVersion = !this.PythonVersion ? [] : [
      {
        uses: 'actions/setup-Python@v1',
        with: { 'Python-version': this.PythonVersion },
      },
    ];
  ...
  private processDeps(options: PythonProjectCommonOptions) {
  ...
  public postSynthesize(outdir: string) {
    ...
    // `Python_modules/projen` symlink so that yarn won't hate us.
    const projenModule = path.resolve('Python_modules', 'projen');
    try {
  ...
  private resolveDependencies(outdir: string) {
    ...
    const resolveDeps = (current: {[name: string]: string}, user: Record<string, string>) => {
      ...
      for (const [name, currentDefinition] of Object.entries(user)) {
        // find actual version from Python_modules
  ...
  private addDefaultGitIgnore() {
    ...
    this.gitignore.exclude(
      ...
      '# Diagnostic reports (https://Pythonjs.org/api/report.html)',
      ...
      '# Compiled binary addons (https://Pythonjs.org/api/addons.html)',
      'build/Release',
...
export interface PythonBuildWorkflowOptions {
  /**
   * @default - default image
   */
...
export class PythonBuildWorkflow extends GithubWorkflow {
  public readonly buildJobId: string;

  constructor(project: PythonProject, name: string, options: PythonBuildWorkflowOptions) {
    super(project, name);
    ...
    const job: any = {
      'runs-on': 'ubuntu-latest',
      'env': {
        CI: 'true', // will cause `PythonProject` to execute `yarn install` with `--frozen-lockfile`
      },
      'steps': [
        // bootstrap
